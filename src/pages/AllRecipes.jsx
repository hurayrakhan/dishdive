            ...
            if (
                ...
                pages.push(
                    <button
                        key={i}
                        onClick={() => paginate(i)}
                        className={`px-3 py-1 rounded-md text-sm font-medium ${i === currentPage ? 'bg-green-600 text-white' : 'bg-white text-gray-700 border border-gray-300'
                            }`}
                    >
                        {i}
                    </button>
                                    ...
                                    <div className={`relative bg-white rounded-3xl border border-gray-200 group overflow-hidden shadow-md hover:shadow-blue-400 transition-all duration-500 ${gridView ? 'h-[380px]' : 'flex h-[280px]'}`}>
                                        <div className={`${gridView ? '' : 'w-1/3'}`}>
                                            <img
                                                ...
                                                className={`${gridView ? 'w-full h-56 object-cover' : 'w-full h-full object-cover'
                                        ...
                                        <div className="absolute bottom-4 right-4 flex flex-col items-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                                            <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-full shadow-md">
                                                ...
                                                <button
                                                    ...
                                                    className={`p-2 rounded-full shadow transition-all duration-300 ${likedIds.includes(recipe._id) ? 'bg-red-500 text-white' : 'bg-white text-red-500 hover:bg-red-100'
                                                        }`}
                                                >
                                                    <FaHeart />
                                                </button>
                                            ...
                                            <button
                                                ...
                                                className={`p-2 rounded-full shadow transition-all duration-300 ${wishlist.includes(recipe._id) ? 'bg-yellow-400 text-white' : 'bg-white text-yellow-500 hover:bg-yellow-100'
                                                    }`}
                                            >
                                                <FaStar />
                                            </button>
